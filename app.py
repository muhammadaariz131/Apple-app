    ...
    if "FILLED" in action_type:
        ...
        if st.button(f"✅ Submit {action_type.split(' ')[1]}", type="primary", use_container_width=True):
            if not driver_name or not location_name:
                ...
                simulated_lat = 34.0 + random.uniform(-0.05, 0.05)
                simulated_lon = 74.8 + random.uniform(-0.05, 0.05)
                img_bytes = receipt_photo.getvalue()
                
                for index, row in edited_df.iterrows():
                    if row["Boxes"] > 0:
                        insert_log(action_type, driver_name, location_name, row["Variety"], row["Boxes"], simulated_lat, simulated_lon, img_bytes)
    ...
    # --- ACTION 3 & 4: Handling EMPTY Crates (No Variety Needed) ---
        ...
        if st.button(f"✅ Submit {action_type.split(' ')[1]}", type="primary", use_container_width=True):
             if not driver_name or not location_name:
                 ...
                 insert_log(action_type, driver_name, location_name, "N/A (Empty)", empty_crates, simulated_lat, simulated_lon, img_bytes)
                 st.success(f"✅ Success! Data recorded.")
...
# --- 4. ADMIN DASHBOARD ---
    ...
    if admin_pin != "1234":
        if admin_pin: st.error("Incorrect PIN.")
