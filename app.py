...
def delete_log(entry_id):
    ...
    conn = sqlite3.connect('apple_harvest.db')
    c = conn.cursor()
    c.execute("DELETE FROM logistics_log WHERE id = ?", (entry_id,))
    conn.commit()
    conn.close()
...
# --- 4. ADMIN DASHBOARD ---
    ...
    if not st.session_state.admin_logged_in:
        ...
        # Removed the password hint entirely for security
        st.warning("🔒 This section is restricted to Admin use only.")
    ...
    if st.session_state.admin_logged_in:
        ...
        # --- 3. DELETE ENTRY SYSTEM ---
        st.divider()
        st.subheader("🗑️ Delete a Record")
        st.error("⚠️ Warning: Deleting a record is permanent and cannot be undone.")
        
        # Create a label that includes the unique ID so we delete the exact right one
        df['delete_label'] = "ID: " + df['id'].astype(str) + " | " + df['timestamp'] + " - " + df['driver_name'] + " (" + df['action_type'] + ")"
        
        record_to_delete = st.selectbox("Select a record to permanently delete:", ["-- Select a record --"] + df['delete_label'].tolist())
        
        if record_to_delete != "-- Select a record --":
            if st.button("🚨 Delete This Record"):
                # Extract just the ID number from the dropdown text
                selected_id = int(record_to_delete.split(" | ")[0].replace("ID: ", ""))
